  ...
  Hindari payload yang bertingkat terlalu dalam; utamakan kejelasan dibandingkan kerapatan data.
    ...
    **Contoh:**
    ...
    ✅ Payload datar:
      ...
      ```
* Design is not UI: APIs harus tetap intuitif meskipun tidak memiliki tampilan visual.
...
* Gunakan bentuk jamak (`/users`, `/orders`) kecuali resource memang unik (`/settings`).
* 💡 *Use Case:* Mengakses order milik pelanggan: `GET /api/v1/commerce/customers/123/orders/456
* 💡 *Use Case:* Melakukan beberapa proses terhadap order:
  ```http
...
* Jika ada versi lama yang ingin dihentikan, konsumen API harus diberi waktu minimal 6 bulan sebelum dihentikan sepenuhnya, sehingga tidak ada yang namanya perubahan secara mendadak dan konsumen tidak terganggu.
...
* `X-Telkom-User-Id` bisa disisipkan oleh sistem autentikasi sebelum mencapai backend, dan digunakan sebagai identitas utama untuk otorisasi (applicable untuk SSO).
