...
* Google AIP:
  ...
  * [AIP-188](https://google.aip.dev/188) – Consistency Across Versions. Saat merilis versi baru (v2), endpoint dan resource naming harus konsisten sebisa mungkin agar   konsumen mudah migrasi. Hindari perubahan drastis pada struktur URI tanpa alasan kuat. Pertahankan semantik field & behavior.
    - `/api/v1/auth/login-histories` -> `/api/v2/auth/login-histories`
    - `/api/v1/products/ecosystems` -> `/api/v2/products/partners` (ada perubahan terminologi bisnis ecosystem menjadi partner)
---
